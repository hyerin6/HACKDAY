...
public interface SubsRepository extends JpaRepository<Subscription, Long> {
	//내가 구독한 사람
	@Query("SELECT new com.hackday.timeline.subscription.vo.SubsVO(b.subsNo, c.userNo, c.userId, c.userName, b.regDate) "
	...
	//나를 구독한 사람
	@Query("SELECT new com.hackday.timeline.subscription.vo.SubsVO(b.subsNo, a.userNo, a.userId, a.userName, b.regDate) "
		+ "FROM Subscription b INNER JOIN b.member a "
		+ "INNER JOIN b.subsMember c "
		+ "WHERE c.userNo = ?1 "
		+ "ORDER BY a.regDate DESC")
